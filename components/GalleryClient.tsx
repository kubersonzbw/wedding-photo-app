...
export default function GalleryClient({ initialSlug = DEFAULT_SLUG, initialCode = "" }: { initialSlug?: string; initialCode?: string }) {
  ...
  const uploadHref = `/wedding/${encodeURIComponent(slug)}${guestCode ? `?code=${encodeURIComponent(guestCode)}` : ""}`;

  const load = useCallback(async (nextSlug = slug, nextGuestCode = guestCode) => {
    setHasRequested(true); setLoading(true); setError("");
    try {
      const res = await fetch("/api/gallery", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: nextSlug, guestCode: nextGuestCode }) });
  ...
  return <WeddingShell wide screen>
    <header className="mobile-topbar"><Link href={uploadHref} aria-label="Dodaj zdjęcia">☰</Link><span>NATALIA & ROBERT</span><span aria-hidden="true">♥</span></header>
    ...
    {!initialCode && <section className="gallery-code-card"><div className="floating-field"><label htmlFor="guestCode">Kod weselny</label><input id="guestCode" value={guestCode} onChange={(e)=>setGuestCode(e.target.value)} placeholder="Wpisz kod weselny" /></div><button className="btn btn-ghost" onClick={()=>load()} disabled={loading || !guestCode}>{loading ? "Przygotowujemy galerię…" : "Pokaż galerię"}</button></section>}
